  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        # Host tools shared by every shell (Rust NIF + Nerves cross-build deps).
      ...
      in
      {
        # Default: the project toolchain (Erlang 29 / Elixir 1.20).
        devShells.default = mkShell' erlang elixir;

        # Firmware cross-build shell: Erlang 28 / Elixir 1.19. Nerves enforces
        # host==target OTP major, and no Nerves system ships OTP 29 yet
        # (nerves_system_rpi0_2 2.0.x is OTP 28.5). Build the examples/ firmware
        # here — `nix develop .#firmware` — while the library stays on OTP 29.
        # Drop this shell once Nerves ships an OTP 29 system.
        devShells.firmware = mkShell' pkgs.beam28Packages pkgs.beam28Packages.elixir_1_19;
